...
- 提供一个独立 APK
- 只作用于 `com.miui.home`
- 一个开关控制是否启用，并提供三档触发灵敏度

## 发布状态

- 当前版本：`1.1.1`
- 变更摘要：现代 libxposed 迁移、Android 16 适配、触发灵敏度三档配置
...
- 现代兜底路径提供保守、标准、灵敏三档；标准档要求按住约 `700ms` 到 `1800ms`，且横向位移至少 `300f`，避免和系统普通返回手势的 `180f` 触发阈值混淆。
- 任务切换时优先从 `RecentsModel.getTaskList()` 找当前任务和下一个任务，找不到再回退到旧的 `getSmartRecentsTaskLoadPlan()` 路径。
- 启动动画资源缺失时会退回基础启动参数，避免不同桌面版本资源名变化导致任务启动失败。
...
1. 用户在设置页打开开关并选择灵敏度，应用把状态写入自己的私有偏好文件。
...
7. 如果没有找到可切换任务，会完成手势收尾但不额外震动；如果找到目标但启动失败，才触发失败震动。
...
- 当前系统桌面普通返回箭头阈值约为 `180f`，QuickBack 现代兜底默认标准档使用更高的 `300f` 位移和 `700ms` 停顿来降低误触。
